{ hostname, lib, pkgs, config, options, ... }:

{
  ...
  boot.initrd.luks.cryptoModules = let
  ...
  services.syslogd.enable  = lib.mkForce false;
  services.rsyslogd.enable = lib.mkForce false;
  services.locate.enable   = lib.mkForce false;

  systemd.tmpfiles.rules = [
    "z /var/log/journal 0000 root root -"
  ];
