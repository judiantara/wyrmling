...
let
  hpFirmware = ./firmware/sihp1020.dl;
in {
 # Enable CUPS to print documents.
  services.printing = {
    drivers = with pkgs; [
      foo2zjs
      (pkgs.writeTextDir "share/cups/model/HP-LaserJet-1020.ppd" ''
        *PPD-Adobe: "4.3"
        *FormatVersion:	"4.3"
        *FileVersion:	"1.1"
        ...
        *FoomaticRIPCommandLine: "${pkgs.foo2zjs}/bin/foo2zjs-wrapper -z1 -P -L0 %A"
  ...
  services.udev.extraRules = ''
    SUBSYSTEMS=="usb", ACTION=="add",  ATTRS{idVendor}=="03f0", ATTRS{idProduct}=="2b17", MODE:="0666", GROUP:="lp"
    SUBSYSTEMS=="usb", ACTION=="bind", ATTRS{idVendor}=="03f0", ATTRS{idProduct}=="2b17", RUN+="${pkgs.writers.writePython3 "load-firmware-hplj1020" { libraries = [ pkgs.python3Packages.pyusb ]; } ''
      import usb.core
      import usb.util

      dev = usb.core.find(idVendor=0x03f0, idProduct=0x2b17)
      if dev is not None:
          try:
          ...
          dev.write(1, firmware_data, timeout=5000)
          print("Firmware for printer HP Laserjet 1020 loaded.")
      else:
          print("No HP Laserjet 1020 printer found.")
    ''}"
  '';
    ...
    ensurePrinters = [
      {
        ...
        deviceUri = "usb://HP/LaserJet%201020?serial=JL1YR1S";
        model = "HP-LaserJet-1020.ppd";
        ppdOptions = {
          PageSize = "A4";
